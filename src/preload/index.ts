...
contextBridge.exposeInMainWorld("qvacAPI", {
  ...
  saveFrame: (buffer: ArrayBuffer): Promise<string> =>
    ipcRenderer.invoke("save-frame", buffer),
  ...
  listActions: (): Promise<{ id: string; label: string }[]> =>
    ipcRenderer.invoke("list-actions"),
  ...
  listVoiceCommands: (): Promise<VoiceCommand[]> =>
    ipcRenderer.invoke("list-voice-commands"),
  ...
  onVoiceEvent: (
    ...
    const listener = (
      ...
      voiceEvent: VoiceSessionEvent,
    ): void => callback(voiceEvent);
